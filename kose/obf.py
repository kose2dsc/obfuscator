...
strings = "abcdefghijklmnopqrstuvwxyz0123456789"  # Attention ne pas toucher !
...
def kramer(content: str, key: int) -> str:
    ...
    _names_ = ["_eval", "_exec", "_byte", "_bytes", "_bit", "_bits", "_system", "_encode", "_decode", "_delete", "_exit", "_rasputin", "_kose"]
...
def main():
    ...
    print("\n"*2)
    print(Colorate.Diagonal(Colors.red_to_blue, Center.XCenter(text)))
    print("\n"*5)


    _file = Write.Input("Glissez votre fichier python ici -> ", Colors.red_to_yellow, interval=0.005)



    if not _file.strip() or not isfile(_file):
        Colorate.Error("Ce fichier n'existe pas!")
        return
    ...
    print(Colorate.Diagonal(Colors.red_to_yellow, f"""Cryptage...
Utilisation de la clé {key}...
Séparation...
Encodement...
Géneration de noms random de variables...
Génération de 2 nombres random...
Création des vars...
Ajout des vars...
Finalisation..."""))
    ...
    print(Colorate.Diagonal(Colors.red_to_yellow, """Création du fichier PYC...
Mouvement...
Delete du '__pycache__' folder...
Renomer le fichier PYC..."""))

    print('\n')

    Write.Input("Operation terminé!", Colors.red_to_yellow, interval=0.005)
    return exit()
